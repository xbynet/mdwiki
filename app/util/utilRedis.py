...
redis_decode_client=StrictRedis(connection_pool=_redis_decode_pool,decode_responses=True) #decode_responses=True,ignore_subscribe_messages=True

_redis_pickle_pool=ConnectionPool(host=config.REDIS_HOST,port=config.REDIS_PORT,db=0)#decode_responses=True
redis_pickle_client=StrictRedis(connection_pool=_redis_pickle_pool) #decode_responses=True,ignore_subscribe_messages=True
